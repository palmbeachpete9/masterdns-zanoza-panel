...
func newServerManager(runtimeDir string) *serverManager {
	bin := envDefault("MASTERDNS_BIN", "/usr/local/bin/masterdns-server")
...
func (m *serverManager) ensureServerConfig() error {
	...
	dnsHost := envDefault("ZANOZA_DNS_HOST", "0.0.0.0")
	dnsPort := "53"
	if v := os.Getenv("ZANOZA_DNS_PORT"); v != "" {
		if port, err := strconv.Atoi(v); err == nil && port > 0 && port <= 65535 {
			dnsPort = strconv.Itoa(port)
		} else {
			log.Printf("invalid ZANOZA_DNS_PORT=%q, using default", v)
		}
	}
	dnsUpstream := `["1.1.1.1:53", "1.0.0.1:53"]`
	if v := os.Getenv("ZANOZA_DNS_UPSTREAM"); v != "" {
		var parsed []string
		if err := json.Unmarshal([]byte(v), &parsed); err == nil && len(parsed) > 0 {
			if clean, err := json.Marshal(parsed); err == nil {
				dnsUpstream = string(clean)
			}
		} else {
			log.Printf("invalid ZANOZA_DNS_UPSTREAM=%q, using default", v)
		}
	}
...
LOG_LEVEL = "INFO"
`, m.keyringPath, dnsHost, dnsPort, dnsUpstream)
