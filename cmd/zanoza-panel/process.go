...
func (m *serverManager) ensureServerConfig() error {
	...
	host := envDefault(EnvDNSHost, "0.0.0.0")
	port := dnsPortFromEnv()
	upstream := dnsUpstreamFromEnv()
...
LOG_LEVEL = "INFO"
`, m.keyringPath, host, port, upstream)
	return os.WriteFile(m.configPath, []byte(tmpl), 0o644)
}

// dnsPortFromEnv returns ZANOZA_DNS_PORT when it is a valid 1..65535 port, else
// the default "53" (logging the rejected value). A bad env never breaks the
// generated server_config.toml.
func dnsPortFromEnv() string {
	v := os.Getenv(EnvDNSPort)
	if v == "" {
		return "53"
	}
	...
	log.Printf("invalid %s=%q (want 1..65535), using 53", EnvDNSPort, v)
	return "53"
}

// dnsUpstreamFromEnv parses ZANOZA_DNS_UPSTREAM as a JSON array of "host:port"
// strings and re-marshals it to a canonical TOML/JSON array. On any parse error
// or empty list it falls back to the Cloudflare defaults and logs.
func dnsUpstreamFromEnv() string {
	const fallback = `["1.1.1.1:53", "1.0.0.1:53"]`
	v := os.Getenv(EnvDNSUpstream)
	if v == "" {
		return fallback
	}
	var servers []string
	if err := json.Unmarshal([]byte(v), &servers); err != nil || len(servers) == 0 {
		log.Printf("invalid %s=%q (want JSON array of \"host:port\"), using defaults", EnvDNSUpstream, v)
		return fallback
	}
	raw, err := json.Marshal(servers)
	if err != nil {
		log.Printf("invalid %s=%q (%v), using defaults", EnvDNSUpstream, v, err)
		return fallback
	}
	return string(raw)
}
