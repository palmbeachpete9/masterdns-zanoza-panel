...
func main() {
	...
	if cfg.KeyringFile != "" {
		...
		keyResolver, err = keyring.Load(keyringPath)
		if err != nil {
			log.Errorf("\u274C <red>Keyring Setup Failed</red> <magenta>|</magenta> <cyan>%v</cyan>", err)
			if !*nowaitFlag {
				waitForExitInput()
			}
			os.Exit(1)
		}
		cfg.Domain = keyResolver.Domains()
		log.Infof("\U0001F511 <green>Keyring loaded: <cyan>%d</cyan> domain(s) from <cyan>%s</cyan></green>", len(cfg.Domain), keyringPath)
	}

	var codec *security.Codec
	var keyInfo security.EncryptionKeyInfo
	if keyResolver == nil {
		var kerr error
		keyInfo, kerr = security.EnsureServerEncryptionKey(cfg)
		if kerr != nil {
			log.Errorf("\u274C <red>Encryption Key Setup Failed</red> <magenta>|</magenta> <cyan>%v</cyan>", kerr)
			if !*nowaitFlag {
				waitForExitInput()
			}
			os.Exit(1)
