...
type ServerConfig struct {
	...
	// KeyringFile (zanoza-panel fork) points at a keyring.json describing
	// per-domain keys. When set, the server serves every domain listed in
	// the keyring and selects decryption keys by domain instead of using a
	// single global ENCRYPTION_KEY_FILE.
	KeyringFile                       string   `toml:"KEYRING_FILE"`
